...
def xpath_row_to_cell(addr: str):
...
def check_edge(driver, graph, edge):
    ...
    if method == "get":
        ...
        if graph.data['form_urls'][purl.path] > 10:
            logging.info("FROM ACTION URL (path) %s, visited more than 10 times, mark as done" % str(
                edge.n2.value.url))
            return False
        else:
            return True
...
def follow_edge(driver, graph, edge):
    ...
    if method == "get":
        ...
        raise Exception(
            "Can't handle method (%s) in next_unvisited_edge " % method)
    ...
    try:
        if do.event == "onclick" or do.event == "click":
            ...
            ActionChains(driver).move_to_element(
                el).move_to_element(body).perform()
...
def update_value_with_js(driver, web_element, new_value):
    try:
        ...
        driver.execute_script(
            "arguments[0].value = '"+new_value+"'", web_element)
    except Exception as e:
        logging.error(e)
...
def form_fill(driver, target_form):
    ...
    for el in elem:
                ...
                if form_iel in target_form.inputs:
                    ...
                    if iel.get_attribute("type") == "submit" or iel.get_attribute("type") == "image":
                        ...
                        if iel.get_attribute("maxlength"):
                            try:
                                driver.execute_script(
                                    "arguments[0].removeAttribute('maxlength')", iel)
                            except Exception as e:
                                logging.warning(
                                    "[inputs] faild to change maxlength " + str(form_iel))
                        try:
                            iel.clear()
                            iel.send_keys(i.value)
                        except Exception as e:
                            logging.warning(
                                "[inputs] faild to send keys to " + str(form_iel) + " Trying javascript")
                            try:
                                driver.execute_script(
                                    "arguments[0].value = '"+str(i.value)+"'", iel)
                            except Exception as e:
                                logging.error(e)
                                logging.error(traceback.format_exc())
                                logging.error(
                                    "[inputs] also faild with JS " + str(form_iel))
                        ...
                        logging.warning(
                            "[inputs] using default clear/send_keys for " + str(form_iel))
                        try:
                            iel.clear()
                            iel.send_keys(i.value)
                        except Exception as e:
                    ...
                    for option in options:
                        if option.get_attribute("value") == i.selected:
                            try:
                                ...
                                logging.error(
                                    "Could not click on " + str(form_select) + ", trying JS")
                                update_value_with_js(
                                    driver, select, i.selected)
                            break
            else:
        ...
        for iframe in iframes:
            form_iframe = Classes.Form.Element(
                "iframe", iframe.get_attribute("id"), "")
                ...
                if form_submit.use:
                    try:
                        ...
                        logging.warning("Cannot click on invisible submit button: " + str(
                            submit_button) + str(target_form) + " trying JavaScript click")
                        logging.info(
                            "form_fill Javascript submission of form after failed submit button click")

                        driver.execute_script(
                            "arguments[0].click()", selenium_submit)
                        ...
                        logging.warning(
                            "Cannot click on submit button: " + str(submit_button) + str(target_form))
                        logging.info(
                            "form_fill Javascript submission of form after failed submit button click")
                        el.submit()
    ...
    for source in target_form.sources:
        ...
        if web_element.get_attribute("maxlength"):
            try:
                ...
                logging.warning(
                    "[inputs] faild to change maxlength " + str(web_element))
        ...
        try:
            ...
            try:
                ...
                logging.error(traceback.format_exc())
                logging.error(
                    "[inputs] also faild with JS " + str(web_element))
        ...
        for form_input in form.inputs:
            if form_input.itype == "password":
                ...
                if len(form.inputs) > max_input_for_login:
                    logging.info(
                        "Too many inputs for a login form, " + str(form))
                    continue
